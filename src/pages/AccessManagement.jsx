...
import { Shield, Plus, Pencil, Trash2, UserCheck, UserX, CheckSquare, Square } from 'lucide-react';
...
function AccessForm({ initial, onSave, onClose }) {
  ...
  const selectAll = () => setForm(f => ({ ...f, allowed_modules: ALL_MODULES.map(m => m.key) }));
  const clearAll  = () => setForm(f => ({ ...f, allowed_modules: [] }));

  const handleSave = async () => {
    if (!form.user_email.trim()) { toast.error('Informe o e-mail do usuário'); return; }
    setSaving(true);
    await onSave(form);
    setSaving(false);
  };
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex gap-2">
            <Button variant="ghost" size="sm" className="h-6 text-xs px-2 gap-1" onClick={selectAll}>
              <CheckSquare className="w-3 h-3" /> Todos
            </Button>
            <Button variant="ghost" size="sm" className="h-6 text-xs px-2 gap-1" onClick={clearAll}>
              <Square className="w-3 h-3" /> Nenhum
            </Button>
          </div>
        </div>
        <div className="grid grid-cols-2 gap-2 border border-border rounded-lg p-3 bg-muted/20 max-h-60 overflow-y-auto">
          {ALL_MODULES.map(m => {
            const checked = form.allowed_modules.includes(m.key);
            return (
              <label
                key={m.key}
                className="flex items-center gap-2 cursor-pointer select-none text-sm rounded px-2 py-1 hover:bg-accent/50"
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Controle quais usuários têm acesso ao sistema e quais módulos podem utilizar
          </p>
        </div>
        {isAdmin && (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="border border-purple-200 bg-purple-50 rounded-lg p-4">
          ...
          <p className="text-xs text-purple-600">Acesso total: leitura, inclusão, alteração e exclusão de registros em todos os módulos autorizados.</p>
        </div>
        <div className="border border-blue-200 bg-blue-50 rounded-lg p-4">
          ...
          <p className="text-xs text-blue-600">Somente leitura: pode visualizar informações mas não pode incluir, alterar ou excluir registros.</p>
        </div>
      </div>
      ...
      <div className="flex gap-3 flex-wrap">
        <Input
          ...
          className="max-w-72"
        />
        <span className="text-sm text-muted-foreground self-center">{filtered.length} acesso(s) encontrado(s)</span>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/30">
                <th className="p-3 text-left font-semibold">Usuário (E-mail)</th>
                ...
                <th className="p-3 text-left font-semibold">Obs</th>
              ...
              {filtered.map((item, idx) => {
                ...
                return (
                  <tr key={item.id} className={`border-b border-border last:border-0 hover:bg-muted/10 ${idx % 2 === 1 ? 'bg-accent/10' : ''}`}>
                    ...
                    <td className="p-3 text-xs text-muted-foreground">{item.notes || '—'}</td>
                    {isAdmin && (
                      <td className="p-3">
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={isAdmin ? 6 : 5} className="text-center py-12 text-muted-foreground">
                    ...
                    {isAdmin && <Button variant="link" className="mt-2" onClick={() => { setEditing(null); setDialogOpen(true); }}>Criar o primeiro acesso</Button>}
                  </td>
                </tr>
              )}
            </tbody>
