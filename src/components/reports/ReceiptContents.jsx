...
export function EscritorioHoleriteContent({ employee, entry, month, company, paymentStatus }) {
  ...
  const calc = calculateEscritorioPayroll({
    ...
    fixed_transport_voucher_working_days: entry?.fixed_transport_voucher_working_days ?? 0,
  ...
  const outrosBeneficios = [
    ...
    { label: `Vale Transporte Fixo (${entry?.fixed_transport_voucher_worked_days ?? 0}d × ${formatCurrency((entry?.fixed_transport_voucher ?? 0) > 0 && (entry?.fixed_transport_voucher_working_days ?? 0) > 0 ? (entry.fixed_transport_voucher / entry.fixed_transport_voucher_working_days) : 0)})`, value: calc.fixed_transport_voucher, show: calc.fixed_transport_voucher > 0 },
